import { Card } from "@/components/ui/card"
import { Avatar, AvatarFallback } from "@/components/ui/avatar"
import Link from "next/link"

function getPerformanceColor(score: number): string {
  if (score >= 7.5) {
    return "text-green-500"
  } else if (score >= 5.0) {
    return "text-yellow-500"
  } else {
    return "text-red-500"
  }
}

export function TeamView() {
  const engineers = [
    {
      id: 1,
      name: "Sarah Chen",
      role: "Senior Engineer",
      prs: 12,
      performance: 8.7,
      bugsGenerated: 3,
    },
    {
      id: 2,
      name: "Marcus Johnson",
      role: "Tech Lead",
      prs: 15,
      performance: 8.2,
      bugsGenerated: 4,
    },
    {
      id: 3,
      name: "Emma Rodriguez",
      role: "Engineer",
      prs: 18,
      performance: 9.1,
      bugsGenerated: 2,
    },
    {
      id: 4,
      name: "Alex Kim",
      role: "Engineer",
      prs: 10,
      performance: 7.8,
      bugsGenerated: 5,
    },
    {
      id: 5,
      name: "Jordan Smith",
      role: "Senior Engineer",
      prs: 8,
      performance: 6.5,
      bugsGenerated: 6,
    },
  ]

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {engineers.map((engineer) => (
        <Link key={engineer.id} href={`/engineers/${engineer.id}`}>
          <Card className="p-6 bg-secondary/50 border-border hover:bg-secondary/70 transition-colors cursor-pointer h-full">
            <div className="flex items-center gap-4 mb-4">
              <Avatar className="h-12 w-12">
                <AvatarFallback className="bg-primary text-primary-foreground">
                  {engineer.name
                    .split(" ")
                    .map((n) => n[0])
                    .join("")}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-foreground truncate">{engineer.name}</h3>
                <p className="text-sm text-muted-foreground truncate">{engineer.role}</p>
              </div>
            </div>

            <div className="p-4 rounded-lg bg-secondary/30 border border-border mb-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Performance Score</span>
                <span className={`text-2xl font-bold ${getPerformanceColor(engineer.performance)}`}>
                  {engineer.performance.toFixed(1)}
                </span>
              </div>
            </div>

            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">PRs Merged</span>
                <span className="text-foreground font-medium">{engineer.prs}/month</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Bugs Generated</span>
                <span className="text-foreground font-medium">{engineer.bugsGenerated}</span>
              </div>
            </div>
          </Card>
        </Link>
      ))}
    </div>
  )
}
